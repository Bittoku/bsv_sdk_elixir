defmodule BSV.Tokens.Script.ReaderPushDataTest do
  ...
  # ---------------------------------------------------------------------------
  # The STAS 3.0 canonical engine base template hex (copied from Stas3Builder).
  # The template is 2899 bytes; its first 4 bytes are the engine prefix
  # (0x6D 0x82 0x73 0x63) and its last byte is OP_RETURN (0x6A).
  # SHA-256: 5c659f5f3abdad612c4bfd19b6034f2df0c0bcef1af1ca928d0f5a34ac3ee371.
  ...
  Layout:
    0x14 | owner(20) | action_data_bytes | base_template(2899) | 0x14 | redemption(20) | flags_push

  The base template's last byte is OP_RETURN (0x6A), so the parser sees:
    2898 bytes of template body | 0x6A | OP_RETURN payload (redemption + flags)
